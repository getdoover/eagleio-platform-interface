...
from .app_ui import EagleioPlatformInterfaceUI
from .app_state import EagleioPlatformInterfaceState

log = logging.getLogger()

class EagleioPlatformInterfaceApplication(Application):
    config: EagleioPlatformInterfaceConfig  # not necessary, but helps your IDE provide autocomplete!

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

        self.started: float = time.time()
        self.ui: EagleioPlatformInterfaceUI = None
        self.state: EagleioPlatformInterfaceState = None

    async def setup(self):
        self.ui = EagleioPlatformInterfaceUI()
        self.state = EagleioPlatformInterfaceState()
